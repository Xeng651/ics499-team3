...
class SalaryRepo extends Database {
    ...
    public function setSalary($salary) {
        $query = "INSERT INTO `salary`(gross_pay, allowance, deduction, bank_name, net_pay, `date`, employee_id, dept_id) VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $this->connect()->prepare($query);
        ...
        $stmt->execute([$grossPay, $allowance, $deduction, $bankName, $netPay, $date, $employeeID, $deptID]);
    }
    ...
    public function editSalary($salary, $salaryID) {
        $query = "UPDATE `salary` SET gross_pay = ?, allowance = ?, deduction = ?, bank_name = ?, net_pay = ?, `date` = ?, employee_id = ?, dept_id = ? WHERE salary_id = ?";
        $stmt = $this->connect()->prepare($query);
        $grossPay = $salary->getGrossPay();
        $allowance = $salary->getAllowance();
        $deduction = $salary->getDeduction();
        ...
        $employeeID = $salary->getEmployeeID();
        $deptID = $salary->getDeptID();
        $stmt->execute([$grossPay, $allowance, $deduction, $bankName, $netPay, $date, $employeeID, $deptID, $salaryID]);
    }
}
