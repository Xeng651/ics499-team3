...
class SalaryModel extends Database {
    ...
    protected function setSalary($salary) {
        $query = "INSERT INTO `salary`(gross_pay, bank_name, net_pay, employee_id, dept_id) VALUES(?, ?, ?, ?, ?)";
        $stmt = $this->connect()->prepare($query);
        ...
        $stmt->execute([$grossPay, $bankName, $netPay, $employeeID, $deptID]);
    }
    ...
    protected function editSalary($salary, $salaryID) {
        $query = "UPDATE `salary` SET gross_pay = ?, bank_name = ?, net_pay = ?, employee_id = ?, dept_id = ? WHERE salary_id = ?";
        $stmt = $this->connect()->prepare($query);
        $grossPay = $salary->getGrossPay();
        $bankName = $salary->getBankName();
        $netPay = $salary->getNetPay();
        $employeeID = $salary->getEmployeeID();
        $deptID = $salary->getDeptID();
        $stmt->execute([$grossPay, $bankName, $netPay, $employeeID, $deptID, $salaryID]);
    }
}
